...
GitHub Actions（每日 09:00 HKT cron）
  → scrape.py 逐個跑 fetcher（LCSD / Timable / Brand HK 盛事之都）
...
## 資料來源（2026-07-13 實測後更新）

實測結果：URBTIX 係 JS SPA、Cityline / KKTIX 有 anti-bot（403）、
RTHK 盛事頁只係電視節目存檔——全部唔用。v1 用以下三個已驗證來源：

1. **康文署**：data.gov.hk 官方 XML（events.xml + venues.xml），最穩。
2. **Timable**：頁面內嵌 JSON（Remix loaderData），要瀏覽器 UA。
3. **Brand HK 盛事之都**：政府盛事年表（HKTB 盛事都喺度），server-rendered HTML。

售票平台留作將來擴充（README 記低）。

## 已知風險

1. **Timable / Brand HK 網站改版**會令對應 fetcher 失效——設計上
   自動跳過並用返上次資料，唔影響其他來源。
